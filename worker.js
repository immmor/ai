...
${contract.contract_content.replace(/<script[^>]*>.*?<\/script>/gi, '')}
        ...
        try {
          ...
          if (username) {
            ...
            const totalResult = await DB
              .prepare('SELECT COUNT(*) as count FROM orders WHERE username = ?')
              .bind(username)
              .first();
      ...
      if (path === '/api/recharge' && request.method === 'POST') {
        try {
          ...
          if (result.success && result.meta.changes > 0) {
            ...
            return resJson({ code: 200, msg: '充值成功', balance: user?.balance || 0 });
          } else {
            return resJson({ code: 404, msg: '用户不存在' }, 404);
          }
        } catch (err) {
      ...
      // ========== 人工审核充值确认接口 ==========
      if (path === '/api/recharge/confirm' && request.method === 'POST') {
        try {
          const params = await request.json();
          ...
          if (result.success && result.meta.changes > 0) {
            ...
            return resJson({ success: false, message: '用户不存在' }, 404);
          }
        } catch (err) {
          return resJson({ success: false, message: '充值失败', error: err.message }, 500);
        }
      }

      // ========== 学习VIP购买接口 ==========
      if (path === '/api/learn/vip' && request.method === 'POST') {
        try {
          const params = await request.json();
          ...
          if (user.learn_vip_expire_date && new Date(user.learn_vip_expire_date) > now) {
            newExpireDate = new Date(user.learn_vip_expire_date);
            newExpireDate.setDate(newExpireDate.getDate() + duration);
          } else {
            newExpireDate.setDate(now.getDate() + duration);
          }
          ...
          const result = await DB
            ...
            .bind(amount, newExpireDate.toISOString().slice(0, 19).replace('T', ' '), username)
            .run();
          ...
          if (result.success && result.meta.changes > 0) {
            return resJson({
              ...
              data: {
                ...
                learn_vip_expire_date: newExpireDate.toISOString().slice(0, 19).replace('T', ' ')
              }
            });
          } else {
            return resJson({ code: 500, msg: '购买失败' }, 500);
          }
        } catch (err) {
          return resJson({ code: 500, msg: '购买失败', error: err.message }, 500);
        }
      }

      // ========== 重置流量接口 ==========
      if (path === '/api/quota/reset' && request.method === 'POST') {
        try {
          const params = await request.json();
          const { username } = params;
          ...
          const result = await DB
            .prepare('UPDATE user SET used_quota = 0, quota_reset_date = ? WHERE username = ?')
            .bind(new Date().toISOString().slice(0, 19).replace('T', ' '), username)
            .run();
          
          if (result.success && result.meta.changes > 0) {
            return resJson({ code: 200, msg: '流量重置成功' });
          } else {
            return resJson({ code: 404, msg: '用户不存在' }, 404);
          }
        } catch (err) {
          return resJson({ code: 500, msg: '重置失败', error: err.message }, 500);
        }
      }

      // ========== VIP状态查询接口 ==========
      if (path === '/api/learn/vip/status' && request.method === 'GET') {
        try {
          const username = url.searchParams.get('username');
          ...
          const user = await DB
            .prepare('SELECT username, learn_vip_expire_date FROM user WHERE username = ?')
            .bind(username)
            .first();
          ...
          const now = new Date();
          const expireDate = user.learn_vip_expire_date ? new Date(user.learn_vip_expire_date) : null;
          const isVipValid = expireDate && expireDate > now;
          ...
          return resJson({
            ...
            data: {
              username: user.username,
              learn_vip_expire_date: user.learn_vip_expire_date,
              is_vip: isVipValid,
