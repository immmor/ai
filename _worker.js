    ...
    try {
      ...
      if (path === '/api/pay/build-url' && request.method === 'POST') {
        try {
          ...
          return resJson({
            code: 200,
            msg: '支付URL构建成功',
            data: { pay_url: payUrl, order_no: order_no, amount: amount }
          });
        } catch (err) {
          return resJson({ code: 500, msg: '支付URL构建失败', error: err.message }, 500);
        }
      }
      ...
      if (path === '/api/chat' && request.method === 'POST') {
        try {
          ...
          try {
            ...
            return resJson({ code: 500, msg: '网络请求失败', error: fetchErr.message }, 500);
          }
        } catch (err) {
          console.error('AI API Error:', err);
          return resJson({ code: 500, msg: 'AI接口调用失败', error: err.message }, 500);
        }
      }
      ...
      return resJson({
        ...
        testTips: [
          ...
          'POST /api/chat → AI聊天接口（传{prompt,stream}）'
        ]
      });
