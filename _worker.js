...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      // 路由匹配：/api/stats/dashboard 首页仪表盘聚合统计接口
      if (url.pathname === '/api/stats/dashboard' && request.method === 'GET') {
        try {
          // ---- 用户余额 & 价格套餐分布 ----
          const usersResult = await env.DB
            .prepare('SELECT balance, price_plan, v_expire_date, is_admin, is_agent FROM user')
            .all();
          ...
          // ---- 游戏下注 & 派奖趋势（最近7天）----
          const gameResult = await env.DB
            .prepare("SELECT created_at, bet_amount, payout_amount FROM game_bet WHERE created_at >= datetime('now','-6 days')")
            .all();
          ...
          // ---- 提现统计 & 审核漏斗 ----
          const wResult = await env.DB
            .prepare('SELECT amount, status FROM withdraw')
            .all();
          ...
          // ---- 竞猜下注分布（按状态）----
          const fResult = await env.DB
            .prepare('SELECT status FROM football_bet')
            .all();
          ...
          // ---- 签到活跃（最近7天签到人数）----
          const chkResult = await env.DB
            .prepare("SELECT username, created_at FROM checkin WHERE created_at >= datetime('now','-6 days')")
            .all();
